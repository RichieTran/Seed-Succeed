...
export function BottomNav({ currentView, onNavigate }: BottomNavProps) {
  ...
  return (
    <nav className="bg-white/90 backdrop-blur-sm border-t border-gray-200 px-6 pt-3 pb-6 sticky bottom-0 z-40">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-1.5 py-3 px-8 rounded-2xl transition-colors ${
                isActive
...
function GardenIcon({ active }: { active: boolean }) {
  return (
    <svg className="w-7 h-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={active ? 2.5 : 2} strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 22V12" />
